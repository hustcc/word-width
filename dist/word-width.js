...
function () {
  ...
  */
  function _bisearch(ucs, table) {
    var lbound = 0, 
      ubound = table.length - 1, 
      mid;

    if (ucs < table[0][0] || ucs > table[ubound][1]) return 0;

    while (ubound >= lbound) {
      mid = parseInt((lbound + ubound) / 2);
  ...
  */
  ...
  return function (word) {
    var i = 0, len = word.length, sum = 0;
    for (; i < len; i ++) sum += _charCodeWidth(word.charCodeAt(i));
    return sum;
  };
});